...
def view_ticket(request, t_id,  pk):
    ...
    try:
        ...
        if not data.bus.is_departure:
            ...
            url = 'http://' + request.META['HTTP_HOST'] + f'/view-ticket/{t_id}/{pk}/verify/'
...
def verify_ticket(request, t_id, pk):
    if request.user.is_authenticated and request.user.is_staff:
        try:
            data = Transaction.objects.get(tran_id=t_id, pk=pk, refund=False)
            if not data.bus.is_departure:
                return render(request, 'ticket/ticket_verified.html', {'verified': True})
            raise LookupError

        except:
            context = {

                'msg2': f"This Ticket  {t_id} is Invalid or Does not exists.",
                'verified': False
            }
            return render(request, 'ticket/ticket_verified.html', context)
    return redirect('/')
